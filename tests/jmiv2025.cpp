...
ImageUInt8Ptr computerCASF_hybridSubtree(ImageUInt8Ptr img, double radioAdj, const std::vector<int>& thresholds){
    std::chrono::high_resolution_clock::time_point start, start_all, end, end_all;
    if(PRINT_LOG){
        start = std::chrono::high_resolution_clock::now();
    }
...
int main(int argc, char* argv[]) {
    ...
    start = std::chrono::high_resolution_clock::now();
    auto imgOut5 = computerCASF_subtree(img,radioAdj, thresholds);
    end = std::chrono::high_resolution_clock::now();
    std::cout << "Time our approach_hibrid (subtree): " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << " ms\n";
