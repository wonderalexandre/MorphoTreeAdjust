...
int test1() {
    // Definição da imagem e dos parâmetros
    auto img = getPassatImage();
    double radioAdj = 1.5;

    // Criação das component trees
    AdjacencyRelationPtr adj =std::make_shared<AdjacencyRelation>(img->getNumRows(), img->getNumCols(), radioAdj);
    std::unique_ptr<DefaultFlatZonesGraph> graph = std::make_unique<DefaultFlatZonesGraph>(img, adj);
    std::unique_ptr<DefaultFlatZonesGraph> maxTreeFZGraph = std::make_unique<DefaultFlatZonesGraph>(*graph);
    std::unique_ptr<DefaultFlatZonesGraph> minTreeFZGraph = std::make_unique<DefaultFlatZonesGraph>(*graph);
    ...
    maxTreeFZGraph = std::make_unique<DefaultFlatZonesGraph>(*graph);
    minTreeFZGraph = std::make_unique<DefaultFlatZonesGraph>(*graph);
