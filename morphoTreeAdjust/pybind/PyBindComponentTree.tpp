...
template <typename CNPsType>
py::array_t<int> PyBindComponentTree<CNPsType>::reconstructionNode(NodeCTPtr<CNPsType> node) {
    int n = this->getNumRowsOfImage() * this->getNumColsOfImage();
    auto img_numpy = py::array(py::buffer_info(
        nullptr, sizeof(int), py::format_descriptor<int>::value, 1, {n}, {sizeof(int)}
    ));
    ...
    while (!s.empty()) {
        ...
        for (NodeCTPtr<CNPsType> child : n->getChildren()) {
            s.push(child);
        }
    }
...
template <typename CNPsType>
py::array_t<int> PyBindComponentTree<CNPsType>::recNode(NodeCTPtr<CNPsType> _node) {
    int n = _node->getArea();
    NodeCTPtr<CNPsType> parent = _node->getParent();
